...
test('created product should be present and have proper fields && values', async ({
  request,
}) => {
  //! ARRANGE
  ...
  expect(
    delResponse.status(),
    `Product id = ${productId} was not deleted!`,
  ).toBe(200);
});
...
test('created product is present in the products list', async ({ request }) => {
  ...
  //! Assert
  const foundProd = getProdResponseJson.find(
    (prod: ProductResponse) => prod.id === productId,
  );
  expect(
    foundProd,
    `Product ${productId} was not found in the response`,
  ).toBeDefined();
...
test('updated product is present in the list with updated data', async ({
  request,
}) => {
  //! Arrange
